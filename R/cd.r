...
predict_expand <- function(reg, new.time){
  ...
  original_data <- reg$model[,c(time_var, outcome_var)]

  if(all(new.time %in% original_data[,time_var])) {

    warning("Nothing to impute, returning original data")
    return(original_data %>% arrange(!!parse_quosure(time_var)))

  } else {

  names(original_data) <- c("time", "outcome")
  missing_time <- tlcPack::outersect(new.time, original_data$time)
  temp_data <- data.frame(time=missing_time, outcome=NA)
  new_data <- rbind.data.frame(original_data, temp_data)
